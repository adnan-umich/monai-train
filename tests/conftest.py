...
def get_data(pytestconfig):
    ...
    data_dir = pytestconfig.getoption("image_dir")
...
def transform_pipeline(pytestconfig):
    ...
    image_size = tuple(map(int, pytestconfig.getoption("image_size").split(',')))
    roi_size = tuple(map(int, pytestconfig.getoption("roi_size").split(',')))
